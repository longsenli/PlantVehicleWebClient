	...
	function showall(menu_list, parent) {
		...
		for(var menu in menu_list) {
			...
			if(menu_list[menu].nodes.length > 0) {
				...
				$(li).append("<a href=\"#\"><i class='" + menu_list[menu].icon + "'></i> <span class='nav-label'>" + menu_list[menu].text + "</span><span class='fa arrow'></span></a>");
				var nextParent = $("<ul   class=\"nav nav-second-level\" id='ul"+i+"'></ul>");
				i++;
				...
				$("<li><a class='J_menuItem' href='.."+menu_list[menu].href+"' ><i class=\"fa fa-circle-o\"></i>" +
					menu_list[menu].text +
...
//  	alert()
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe id="iframeHome" class="J_iframe" width="97%" height="100%" name="iframe' + dataIndex + '" width="100%" height="100%"  scrolling="yes" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" ></iframe>';
